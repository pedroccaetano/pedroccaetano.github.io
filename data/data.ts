export const companies = [
  {
    title: "Aliare",
    alt: "Aliare",
    role: "Full Stack Developer",
    skills: ["React", "Node.js", "JavaScript", "HTML5", "CSS3", "Sass", "Git", "PostgreSQL", "MySQL", "RabbitMQ", "Nginx", "Azure DevOps"],
    period: "2021 - Até o momento",
    logo: '/assets/images/logos/aliare.jpg'
  },
  {
    title: "Santri Soluções",
    alt: "Santri Soluções",
    role: "Full Stack Developer",
    skills: ["Vue.js", "PHP", "JavaScript", "jQuery", "Ajax", "HTML5", "CSS3", "GitLab",  "Mysql", "OpenCart", "Apache Tomcat"],
    period: "2018 - 2021",
    logo: '/assets/images/logos/santri.jpg'
  },
  {
    title: "IFG - Instituto Federal de Goiás",
    alt: "IFG",
    role: "Monitor de estrutura de dados",
    skills: ['c++', 'c', "Estrutura de Dados", 'Algoritmos'],
    period: "2017 - 2018",
    logo: '/assets/images/logos/ifg.jpg'
  },
  {
    title: "IBGE - Instituto Brasileiro de Geografia e Estatística",
    alt: "IBGE",
    role: "Estágio Desenvolvedor Web",
    skills: ["Java", "Java Server Faces", "Primefaces", "JavaScript", "HTML5", "CSS3", "Mysql", "Apache Subversion"],
    period: "2016 - 2017",
    logo: '/assets/images/logos/ibge.png'
  }
];

export const institutes = [
  {
    short_title: "FAST - NUCES",
    title: "Faculdade Unyleya",
    alt: "Unyleya",
    role: "Pós-graduação em Arquitetura de Software",
    skills: ["arquitetura de software", "microsserviços", "big data", "DevOps", "Gestão de Projetos"],
    period: "2020 - 2021",
    startingYear: "2020",
    logo: '/assets/images/logos/unyleya.jpg'
  },
  {
    short_title: "IFG - Instituto Federal de Goiás",
    title: "IFG - Instituto Federal de Goiás",
    alt: "IFG - Instituto Federal de Goiás",
    role: "Bacharel em Sistema de Informação",
    skills: ["algoritmos", "banco de dados", "linguagem de programação", "sistemas operacionais"],
    period: "2016 - 2020",
    startingYear: "2016",
    logo: '/assets/images/logos/ifg.jpg'
  }
];

export const courses = [
  {
    short_title: "CodeBoost",
    title: "CodeBoost",
    alt: "CodeBoost",
    role: "Curso de aperfeiçoamento na criação de interfaces de alta qualidade. Utilizando as tecnologias CSS3 (SASS) e Javascript",
    skills: ["HTML5", "CSS3", "Sass", "JavaScript", "gulp.js"],
    period: "2021",
    startingYear: "2020",
    logo: '/assets/images/logos/codeboost.jpg'
  },
  {
    short_title: "GoStack Bootcamp",
    title: "GoStack Bootcamp",
    alt: "GoStack Bootcamp",
    role: "Bootcamp focado no desenvolvimento de aplicações utilizando as tecnologias NodeJS, ReactJS e React Native.",
    skills: ["NodeJS", "React", "React Native"],
    period: "2020",
    startingYear: "2016",
    logo: '/assets/images/logos/rocketseat.jpg'
  }
];


export const skillsArray = [
  {
    name: "Ruby on Rails",
    description: "Web development",
    link: "https://rubyonrails.org/",
    type: "development",
    image: '/assets/images/skills/rails.png'
  },
  {
    name: "React",
    description: "Web development",
    link: "https://reactjs.org/",
    type: "development",
    image: '/assets/images/skills/react.png'
  },
  {
    name: "Ruby",
    description: "Web development, scrapping",
    link: "https://www.ruby-lang.org/en/",
    type: "development",
    image: '/assets/images/skills/ruby.png'
  },
  {
    name: "Python",
    description: "Web development, scrapping",
    link: "https://www.python.org/",
    type: "development",
    image: '/assets/images/skills/python.png'
  },
  {
    name: "Javascript",
    description: "Web development",
    link: "https://developer.mozilla.org/en-US/docs/Web/JavaScript",
    type: "development",
    image: '/assets/images/skills/javascript.png'
  },
  {
    name: "Typescript",
    description: "Web development",
    link: "https://www.typescriptlang.org/",
    type: "development",
    image: '/assets/images/skills/typescript.png'
  },
  {
    name: "Html5",
    description: "Web layouts",
    link: "https://developer.mozilla.org/en-US/docs/Web/Guide/HTML/HTML5",
    type: "development",
    image: '/assets/images/skills/html-5.png'
  },
  {
    name: "Css3",
    description: "Web design",
    link: "https://developer.mozilla.org/en-US/docs/Web/CSS",
    type: "design",
    image: '/assets/images/skills/css3.png'
  },
  {
    name: "Bootstrap",
    description: "Web design",
    link: "https://getbootstrap.com/",
    type: "design",
    image: '/assets/images/skills/bootstrap.png'
  },
  {
    name: "Postgres",
    description: "Database",
    link: "https://www.postgresql.org/",
    type: "database",
    image: '/assets/images/skills/postgresql.png'
  },
  {
    name: "Mysql",
    description: "Database",
    link: "https://www.mysql.com/",
    type: "database",
    image: '/assets/images/skills/mysql.png'
  },
  {
    name: "Redis",
    description: "Database",
    link: "https://redis.io/",
    type: "database",
    image: '/assets/images/skills/redis.png'
  },
  {
    name: "Heroku",
    description: "Devops",
    link: "https://www.heroku.com/",
    type: "devops",
    image: '/assets/images/skills/heroku.png'
  },
  {
    name: "AWS",
    description: "Devops",
    link: "https://aws.amazon.com/",
    type: "devops",
    image: 'assets/images/skills/aws.png'
  },
  {
    name: "Digital Ocean",
    description: "Devops",
    link: "https://www.digitalocean.com/",
    type: "devops",
    image: '/assets/images/skills/digital-ocean.png'
  },
  {
    name: "Nginx",
    description: "Web server",
    link: "https://www.nginx.com/",
    type: "devops",
    image: '/assets/images/skills/nginx.png'
  },
  {
    name: "Visual Studio Code",
    description: "Code editor of choice",
    link: "https://code.visualstudio.com/",
    type: "development",
    image: '/assets/images/skills/vscode.png'
  },
  {
    name: "Figma",
    description: "Collaborative Design",
    link: "https://www.figma.com/",
    type: "design",
    image: '/assets/images/skills/figma.png'
  }
];
